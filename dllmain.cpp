...
signed int __stdcall hPostRender(UObject* pCallObject, UCanvas* pUFunc)
{
	...
	if (ZeroGUI::Window(NULL, &pos, Vector2{ 390.f, 390.f }, menu_open, true))
	{
		...
		ZeroGUI::TextLeft(xorstr("Smelo Beta: " __TIME__ " " __DATE__), Vector2(pos.X + 15, pos.Y + 15), FLinearColor(1.0f, 1.0f, 1.0f, 1.0f), false, true);
		...
		//those are just tabs if i remove ima get 999999 errors
		if (tabs == 0)
		{
...
BOOL InitMain()
{
	...
	uintptr_t GeztBoenrfa = 0;

	std::cout << "GetBoneMatrix: " << "0x" << std::hex << (uintptr_t)reinterpret_cast<decltype(GeztBoenrfa)>((PBYTE)((UINT_PTR)(MemoryHelper::PatternScan("48 8B 05 ? ? ? ? 48 8B 0C C8 48 8B 04 D1")) + *(PINT)((UINT_PTR)(MemoryHelper::PatternScan("48 8B 05 ? ? ? ? 48 8B 0C C8 48 8B 04 D1")) + ((5) - sizeof(INT))) + (5))) - (uintptr_t)GetModuleHandle("FortniteClient-Win64-Shipping.exe") << std::endl;
